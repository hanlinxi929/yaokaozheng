<template>
	<view>
		<view class="wrapper">
			<view class="handBtn">
				<picker-color :isShow="showPickerColor" @callback='getPickerColor' />
				<u-tag :bgColor="selectColor" @click="selectColorEvent()" shape='circle' class="colorBtn"></u-tag>
				<text class="texts">请在上方空白处书写您的签字</text>
				<button @click="retDraw" class="delBtn">清除重写</button>
				<!-- <button @click="saveCanvasAsImg" class="saveBtn">保存</button>
				<button @click="previewCanvasImg" class="previewBtn">预览</button> -->
				<button @click="uploadCanvasImg" class="uploadBtn">提交签名</button>
			...
			uploadCanvasImg() {
				...
				wx.canvasToTempFilePath({
					...
					success(res) {
						...
						//上传
						let avatarfile = res.tempFilePath;
						wx.uploadFile({
							url:  baseUrl +'hs/api/upload', // 仅为接口地址
							filePath: avatarfile,
							name: 'file',
							formData: {
								// user: 'test'
