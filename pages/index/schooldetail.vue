	...
	<view>
		...
		<view id="type-el" class="type-el" v-if="modelShow">
			<scroll-view scroll-y="true" :scroll-into-view="scrollIntoView" @scroll="scrolls"
				...
				<view class="goods_box" id="js">
					<view class="goods_list">
						<image :src="baseUrl+schooldetail.collegeIntroduction" mode="aspectFill"></image>
					</view>
				</view>
				<view class="goods_box" id="cx">
					<view class="goods_list">
						<image :src="baseUrl+schooldetail.registration" mode="aspectFill"></image>
					</view>
				</view>
				<view class="goods_box" id="zg">
					<view class="goods_list">
						<image :src="baseUrl+schooldetail.registrationEligibility" mode="aspectFill"></image>
					</view>
				</view>
				<view class="goods_box" id="ms">
					<view class="goods_list">
						<image :src="baseUrl+schooldetail.entranceInterview" mode="aspectFill"></image>
					</view>
				</view>
				<view class="goods_box" id="fy">
					<view class="goods_list">
						<image :src="baseUrl+schooldetail.otherExpenses" mode="aspectFill"></image>
					</view>
				</view>
			</scroll-view>
		</view>
		...
		<view class="btns width100 bg-white flex justify-between">
			...
			<button class="round btn_right mt_20"
				@click.stop="$api.toPage('index/bmxinxi?oid='+oid+'&oneId='+oneId+'&sid='+sid)">立即报名</button>
		</view>
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			...
			var that = this
			var urls = '/pages/index/schooldetail?sid=' + that.sid + '&oid=' + that.oid +
		...
		mounted() {
			...
			this.$nextTick(() => {
				...
				query
					.select('#js')
					.boundingClientRect((js) => {
						this.detailetab[0].height = parseInt(js.height);
					})
					.exec();
				query
					.select('#cx')
					.boundingClientRect((cx) => {
						this.detailetab[1].height = parseInt(cx.height);
					})
					.exec();
				query
					.select('#zg')
					.boundingClientRect((zg) => {
						this.detailetab[2].height = parseInt(zg.height);
					})
					.exec();
				query
					.select('#ms')
					.boundingClientRect((ms) => {
						this.detailetab[3].height = parseInt(ms.height);
					})
					.exec();
				query
					.select('#fy')
					.boundingClientRect((fy) => {
						this.detailetab[4].height = parseInt(fy.height);
					})
					.exec();
			});
		},
